...
class gdaxWebsocketClientBitcoin(gdax.WebsocketClient):
	...
	def on_message(self, msg):
		...
		conn = sqlite3.connect('gdaxBTC.db')
		#c = conn.cursor()
		#c.execute("INSERT INTO GdaxBTC VALUES (?,?,?,?,?,?,?,?)",line)
		#conn.commit()
		#conn.close()
		with open('1gdaxWS_BTC.csv', 'a', newline='') as csvfile:    
			#Use csv Writer
...
class gdaxWebsocketClientLitecoin(gdax.WebsocketClient):
	...
	def on_message(self, msg):
		...
		#conn.commit()
		#conn.close()
		with open('1gdaxWS_LTC.csv', 'a', newline='') as csvfile:    
			#Use csv Writer
			csvWriter = csv.writer(csvfile, delimiter=',')
			#writes the message
			csvWriter.writerow(line)
